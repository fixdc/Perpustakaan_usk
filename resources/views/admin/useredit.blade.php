...
<div class="h-full ">
    <div class="p-4 sm:ml-64 h-full">
        <div class="p-4 border-2 border-gray-200 border-dashed rounded-lg dark:border-gray-700 h-screen">
            ...
            <form class="mx-auto max-w-5xl flex flex-col justify-start text-white" action="{{ route('edit', [$anggota->f_username]) }}" method="POST" >
                @csrf
                <div class="mb-5 font-Mont font-bold">
                    <label  class="block font-bold mb-2 text-sm text-black">Nama</label>
                    <input type="text" id="name" name="f_nama" value="{{ $anggota->f_nama }}"
                ...
                <div class="mb-5 font-Mont font-bold">
                    ...
                    <input type="text" name="f_username" value="{{ $anggota->f_username }}"
                        class="shadow-sm bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500 dark:shadow-sm-light"
                        required>
                </div>
                <div class="mb-3 font-Mont font-bold">
                    <label for="email" class="font-bold block mb-2 text-sm text-black">Tempat Lahir</label>
                    <input type="text" name="f_tempatlahir" value="{{ $anggota->f_tempatlahir }}"
                        class="shadow-sm bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500 dark:shadow-sm-light"
                        required>
                </div>
                <div class="mb-5 font-Mont font-bold">
                    <label for="email" class="block mb-2 text-sm font-bold text-black">Tanggal Lahir</label>
                    <input type="date" name="f_tanggallahir" value="{{ $anggota->f_tanggallahir }}"
                        class="shadow-sm bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500 dark:shadow-sm-light"
                        required>
                </div>
                <div class="mb-5 text-white bg-red-400 p-3 rounded-md w-fit">
                    <a href="{{ route('editviewpassword', ['f_username' => $anggota->f_username]) }}">Edit Password Here</a>
                </div>
                <button type="submit"
